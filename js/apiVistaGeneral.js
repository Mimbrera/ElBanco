...
function mostrarHTML(usuario) {
    ...
    usuario.forEach(users => {
        const {id, name, website, address, company } = users;
        html +=`
            <tr>
                <td>${name}</td>
                <td>${website}</td>
                ...
                <td class="tabmobil">${company.bs}</td>
                <td class="tabmobil">${company.name}</td>
                <td class="tabmobil">${website}</td>
                <td><button class="button_candidatos" type="submit">Seleccione <img src="../img/triangulo.png" /></button></td>
            </tr>
        `;
